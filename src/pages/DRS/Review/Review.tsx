...
import { Chart as ChartJS, CategoryScale, LinearScale, PointElement, LineElement, Legend, Tooltip, Title, BarElement } from 'chart.js';
import './review.scss';
...
ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, BarElement, Legend, Tooltip, Title);
...
const Review = () => {
  ...
  const { data: quizData, error: fetchQuizError, isLoading: isFetchQuizLoading } = useFetchQuizByIdQuery(quiz?.result?.quizId!, { skip: !quiz });
  ...
  const AccuracyLineChart = <Line options={accuracyOptions} data={AccuracyData} />

  const TimeLineChart = <Line options={timeOptions} data={TimeData} />

  const ResponseBarGraph = <Bar options={responseOptions} data={ResponseData} />
  ...
  const handleResponseChartText = (responses: number[]) => {
    if (responses[0] === responses.reduce((acc, response) => acc + response, 0)) {
      return `You didn't answer any questions. Try to answer all questions.`;
    } else if (responses[1] > responses[2] && responses[1] > responses[3] && responses[1] > responses[4]) {
      return `You were struggling with answering questions correctly. Try to focus on answering questions correctly.`;
    } else if (responses[2] > responses[1] && responses[2] > responses[3] && responses[2] > responses[4]) {
      return `You were doing okay with answering questions correctly. See if you can improve.`;
    } else if (responses[3] > responses[1] && responses[3] > responses[2] && responses[3] > responses[4]) {
  ...
  const textContainerElements = (
    <>
      <p style={{ fontWeight: 700, fontSize: "1.1rem", color: handleDRSHeaderColor(accuracy) }}>{handleDRSHeader(accuracy)}</p>
      <br />
