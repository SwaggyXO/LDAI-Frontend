...
import { getUserCookie } from "../../../features/user/userCookieHandler.ts";
import { useFetchUserQuizzesQuery, useFetchUserResultQuery } from "../../../api/userApiSlice.ts";
...
const Review = () => {
  ...
  let accuracy, timeTaken, responses, questionIndexList, AccuracyData, TimeData, ResponseData, AccuracyLineChart, TimeLineChart, ResponseBarGraph, textContainerElements;

  if (quiz?.data) {
    console.log(quiz.data);

    accuracy = quiz!.data?.score! * 100;
    timeTaken = Number(quiz!.data?.timeTaken);
    responses = quiz!.data?.responses!;
    questionIndexList = responses.map((response, idx) => `Q${idx + 1}`);
    ...
    TimeLineChart = <Line options={timeOptions} data={TimeData} />;
    ...
    <>
      ...
      <div className="top-stats">
        ...
        <div className="stats-container">
          ...
          <div className="stat">
            <div className="stat-container">
              <p>{timeTaken && convertTime(timeTaken)}</p>
            </div>
            <div className="stat-header">
              <p>Time Taken</p>
            </div>
        ...
        <div className="charts-container">
          <div className="chart-container">
            ...
            <div className="chart-details">
              <p>{accuracy && handleAccuracyChartText(accuracy)}</p>
            </div>
          </div>
          ...
          <div className="chart-container">
            ...
            <div className="chart-details">
              <p>{timeTaken && handleTimeChartText(timeTaken)}</p>
            </div>
          </div>
          <div className="learn__features-container__line">
            <section />
          ...
          <div className="chart-container">
            ...
            <div className="chart-details">
              <p>{ResponseData && handleResponseChartText(ResponseData.datasets[0].data)}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      {isLoading && isFetchQuizLoading || isFetchUserResultLoading || isFetchUserQuizzesLoading && <Loader />}
