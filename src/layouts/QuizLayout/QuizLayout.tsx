...
const QuizLayout = () => {
  ...
  const handleExit = async () => {
    const response = await fetch(`https://ldotai-core-ms.azurewebsites.net/api/ldai-core/v1/quiz/quit/${currUser?.userId}`);
  ...
  const content = (
    <>
      <Topnav customButton={<NavButton to="exit" onClick={handleExit} className="cross-button"/>} marbles={marbles} classname="quiz-layout" />
